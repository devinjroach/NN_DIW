...
def train(X, Y, model, loss_function, optim, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        #zeros = torch.zeros((1,196))
        #loss = loss_function(Y_pred[:,0], Y[:,0]) + 0.0000001*(loss_function(Y_pred[:,1], zeros) + loss_function(Y_pred[:,2], zeros) + loss_function(Y_pred[:,3], zeros))
...
plt.plot(xaxis, outputs[:,0], 'r-')
plt.plot(xaxis, outputs_model.detach().numpy()[:,0], 'bo')
plt.plot(xaxis, outputs_model.detach().numpy()[:,1], 'ko', markersize=1.5)
plt.plot(xaxis, outputs_model.detach().numpy()[:,2], 'go', markersize=1.5)
plt.plot(xaxis, outputs_model.detach().numpy()[:,3], 'ro', markersize=1.5)
